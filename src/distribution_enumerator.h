...
typedef struct {
   ...
   * Distribution_Coordinate::min_log_alpha_r. The entry is included to make the
   * distance available to qsort(), as m cannot be passed to qsort(), and as
   * qsort_r() that does take an additional parameter is non-portable in that it
   * has different prototypes on different platforms(!)
   *
