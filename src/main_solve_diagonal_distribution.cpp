  ...
  const int root)
{
  ...
  data[7] = (uint32_t)(arguments->instances);
  data[8] = (uint32_t)(arguments->count);

  result = MPI_Bcast(data, 9, MPI_UNSIGNED, root, MPI_COMM_WORLD);
  ...
  const int root)
{
  ...
  arguments->instances = data[7];
  arguments->count = data[8];
