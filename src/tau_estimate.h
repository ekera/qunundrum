 ...
 *
 * If at least one of the n argument pairs sampled is out of bounds, tau_d and
 * tau_r are both set to MAX_DBL and #FALSE is returned. Otherwise, tau_d and
 * tau_r are valid estimates and #TRUE is returned.
 *
 ...
 *
 * \param[in] distribution      The distribution from which to sample argument
 *                              pairs (alpha_d, alpha_r).
 ...
 *
 * \return  Returns #FALSE if at least one of the n argument pairs sampled is
 ...
 *
 * If at least one of the n arguments alpha sampled is out of bounds, tau is set
...
/*!
 * \brief   Computes tau from n arguments alpha_i for angles phi_i sampled from
 *          the distribution.
 *
 * If at least one of the n argument pairs sampled is out of bounds, tau is set
 * to MAX_DBL and #FALSE is returned. Otherwise, tau is a valid estimate and
 * #TRUE is returned.
 *
 * We define tau as
 ...
 *
 * \param[in] distribution      The distribution from which to sample arguments
 *                              alpha_i for angles phi_i.
 * \param[in, out] random_state The random state to use when sampling.
 * \param[in] n                 The integer n.
 * \param[in] delta_bound       The bound on the offset Delta.
 * \param[in] eta_bound         The bound on the peak index eta.
 * \param[out] tau              The computed value of tau.
 *
 * \return  Returns #FALSE if at least one of the n arguments sampled is out of
