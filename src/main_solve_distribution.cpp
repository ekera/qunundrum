/*!
 * \file    main_solve_distribution.cpp
 * \ingroup solve_distribution_exe
 *
 * \brief   The definition of the main entry point to the
 *          solve_distribution executable, and of associated functions.
 */

/*!
 * \defgroup solve_distribution_exe \
 *           The solve_distribution executable
 * \ingroup  solve_executable
 *
 * \brief    A module for the solve_distribution executable.
 */
...
typedef struct {
   ...
   */
  char * path;
} Solve_Distribution_Arguments_Entry;
...
typedef struct {
   ...
   */
  Solve_Distribution_Arguments_Entry * entries;
} Solve_Distribution_Arguments;
  ...
  char ** argv)
{
  ...
  arguments->entries =
    (Solve_Distribution_Arguments_Entry *)malloc(
      (arguments->count) * sizeof(Solve_Distribution_Arguments_Entry));
  ...
  Solve_Distribution_Arguments * const arguments)
{
  ...
  memset(arguments, 0, sizeof(Solve_Distribution_Arguments));
}
...
static void main_server(
  const Solve_Distribution_Arguments * const arguments,
  const Solve_Distribution_Arguments_Entry * const entry,
  ...
  while (TRUE) {
    ...
    if (MPI_NOTIFY_SAMPLING_FAILED_OUT_OF_BOUNDS == notification) {
      ...
      if (solution[0] == solution_status.n) {
        ...
        time_prepare_system_us   = (uint64_t)solution[3]; /* high 32 bits */
        time_prepare_system_us <<= 32;
        time_prepare_system_us  ^= (uint64_t)solution[4]; /* low 32 bits */
        ...
        time_solve_system_us   = (uint64_t)solution[5]; /* high 32 bits */
        time_solve_system_us <<= 32;
        time_solve_system_us  ^= (uint64_t)solution[6]; /* low 32 bits */
  ...
  const Solve_Distribution_Arguments * const arguments)
{
  ...
  while (TRUE) {
    ...
    for (uint32_t i = 0; i < n; i++) {
      result = distribution_sample_pair_j_k(
                  &distribution,
                  &random_state,
                  js[i],
                  ks[i]);
    ...
    uint32_t solution[7] = {
      n,
      (uint32_t)recovery_status_d,
      (uint32_t)recovery_status_r,
...
/*!
 * \brief The main entry point to the solve_distribution executable.
 *
 * \param[in, out] argc   The arguments count.
 * \param[in, out] argv   The arguments vector.
 *
...
int main(int argc, char ** argv) {
  ...
  Solve_Distribution_Arguments arguments;
  ...
  if (!result) {
    /* Exit gracefully if a error occured. */
    MPI_Finalize();
    return 0;
  }
  ...
  if (MPI_RANK_ROOT == mpi_rank) {
    ...
    distribution_loader_init(&loader, paths, arguments.count);

    /* Process the distributions. */
    for (uint32_t i = 0; i < (arguments.count); i++) {
      Distribution * distribution =
        distribution_loader_pop(&loader);
